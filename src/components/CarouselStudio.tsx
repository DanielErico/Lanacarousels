...
export const CarouselStudio: React.FC<CarouselStudioProps> = ({
  ...
  // Mock Instagram Feed Interactive States
  const [mockLiked, setMockLiked] = useState(false);
  const [mockSaved, setMockSaved] = useState(false);
  const [mockCaptionExpanded, setMockCaptionExpanded] = useState(false);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-white border border-slate-200/90 rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden">
            {/* Instagram Mock Top Bar */}
            <div className="px-4 py-3 bg-white border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                {/* Story Gradient Ring around Avatar */}
                <div className="w-8 h-8 rounded-full p-[1.5px] bg-gradient-to-tr from-amber-500 via-rose-500 to-purple-600 flex items-center justify-center">
                  <div className="w-full h-full rounded-full bg-slate-900 flex items-center justify-center text-white font-bold text-[10px]">
                ...
                <div>
                  <div className="flex items-center space-x-1">
                    <span className="font-bold text-xs text-slate-900 leading-none">
                      {activeBrand.igHandle || 'lana.carousel'}
                    </span>
                    <BadgeCheck className="w-3.5 h-3.5 text-sky-500 fill-sky-500" />
                  </div>
                  <span className="text-[10px] text-slate-400 leading-tight">Original audio • 🎧</span>
                </div>
              </div>
            ...
            {/* Instagram Post Media: 4:5 Slide Container with Interactive Overlay */}
            <div className="relative bg-slate-950 flex items-center justify-center select-none group" style={{ aspectRatio: currentCarousel.platformSpec === 'ig_1_1' ? '1/1' : '4/5' }}>
              {/* Slide Counter Overlay Pill (Top Right) */}
              <div className="absolute top-3 right-3 z-30 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-md text-white text-[11px] font-bold tracking-wider pointer-events-none">
                {activeSlideIndex + 1}/{currentCarousel.slides.length}
              </div>
              ...
              {activeSlideIndex > 0 && (
                <button
                  ...
                  className="absolute left-2.5 top-1/2 -translate-y-1/2 z-30 w-7 h-7 rounded-full bg-white/80 hover:bg-white text-slate-900 shadow-md flex items-center justify-center transition-all hover:scale-105 active:scale-95"
                >
              ...
              {activeSlideIndex < currentCarousel.slides.length - 1 && (
                <button
                  onClick={() => setActiveSlideIndex(prev => Math.min(currentCarousel.slides.length - 1, prev + 1))}
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 z-30 w-7 h-7 rounded-full bg-white/80 hover:bg-white text-slate-900 shadow-md flex items-center justify-center transition-all hover:scale-105 active:scale-95"
                >
            ...
            <div className="px-4 pt-3 pb-2 flex items-center justify-between">
              <div className="flex items-center space-x-3.5">
                <button
                  onClick={() => setMockLiked(prev => !prev)}
                  className="transition-transform active:scale-125"
                >
                  <Heart className={`w-5 h-5 ${mockLiked ? 'text-rose-500 fill-rose-500' : 'text-slate-800 hover:text-slate-600'}`} />
                </button>
              ...
              <div className="flex items-center space-x-1.5">
                {currentCarousel.slides.map((_, idx) => (
                  <button
                    key={idx}
              ...
              <button
                onClick={() => setMockSaved(prev => !prev)}
                className="transition-transform active:scale-125"
              >
                <Bookmark className={`w-5 h-5 ${mockSaved ? 'text-slate-900 fill-slate-900' : 'text-slate-800 hover:text-slate-600'}`} />
              </button>
            </div>

            {/* Instagram Feed Engagement & Caption */}
            <div className="px-4 pb-4 space-y-1.5 text-xs text-slate-800">
              <p className="font-bold text-[11px] text-slate-900">
                {mockLiked ? '1,249 likes' : '1,248 likes'}
              </p>
                ...
                <span>
                  {mockCaptionExpanded ? (
                    <>
                      ...
                      <span className="block mt-1.5 text-sky-700 font-medium">
                        {(currentCarousel.caption.hashtags || []).map(h => (h.startsWith('#') ? h : `#${h}`)).join(' ')}
                      </span>
                    </>
                  ) : (
                    <>
                      {currentCarousel.caption.text.slice(0, 75)}...{' '}
                      <button
                        onClick={() => setMockCaptionExpanded(true)}
              ...
              <p className="text-[10px] text-slate-400 tracking-wider uppercase pt-1">
                2 HOURS AGO
              </p>
            </div>

            {/* Slide Navigation Strip & Downloads */}
            <div className="p-3 bg-slate-50 border-t border-slate-200/80 flex items-center justify-between gap-2">
              ...
              {/* Download Current Slide PNG Button */}
              <button
                onClick={handleExportSinglePng}
                disabled={isExporting}
                className="px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 text-slate-800 text-[11px] font-bold flex items-center gap-1.5 transition-all shadow-sm disabled:opacity-50"
              >
